...
// Παίρνουμε το offer_id από το POST αίτημα
$data = json_decode(file_get_contents('php://input'), true);

if (!isset($data['offer_id'])) {
    echo json_encode(['success' => false, 'error' => 'Offer ID not received']);
    exit();
}
...
if ($result_vehicle->num_rows > 0) {
    ...
    if (canTakeMoreTasks($vehicle_id, $conn)) {
        ...
        $stmt_update->bind_param("iii", $vehicle_id, $rescuer_id, $offer_id);
